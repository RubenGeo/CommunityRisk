    ...
    .controller('FbfController', ['$translate', '$scope', '$css', '$rootScope', '$compile', '$q', 'Authentication', 'Dashboards', 'Data', 'Sources', '$window', '$stateParams', 'cfpLoadingBar', '_',
        function($translate, $scope, $css, $rootScope, $compile, $q, Authentication, Dashboards, Data, Sources, $window, $stateParams, cfpLoadingBar, _) {
            ...
            $scope.load_data = function(pgData) {
                ...
                $scope.chart_show == 'map' ? $('.view-buttons button.btn-map-view').addClass('active') : $('.view-buttons button.btn-tabular').addClass('active');
            ...
            $scope.generateCharts = function(d) {
                ...
                var currentFormat = function(value) {
                    if (meta_format[$scope.metric] === 'decimal0') { return dec0Format(value); } else if (meta_format[$scope.metric] === 'decimal2') { return dec2Format(value); } else if (meta_format[$scope.metric] === 'percentage') { return percFormat(value); }
                };
                ...
                var high_med_low = function(ind, ind_score, group) {
                    ...
                    if (dimensions_scores[ind]) {
                        ...
                        if (isNaN(width)) { return 'notavailable'; } else if (width < d.thresholds[0]) { return 'good'; } else if (width <= d.thresholds[1]) { return 'medium-good'; } else if (width <= d.thresholds[2]) { return 'medium'; } else if (width <= d.thresholds[3]) { return 'medium-bad'; } else if (width > d.thresholds[3]) { return 'bad'; }
                    }
                };
                ...
                $scope.createHTML = function(keyvalue) {
                    ...
                    for (var i = 0; i < $scope.tables.length; i++) {
                        ...
                        if (!meta_icon[record.name]) { var icon = 'modules/dashboards/img/undefined.png'; } else { var icon = 'modules/dashboards/img/' + meta_icon[record.name]; }
                ...
                mapChart
                    ...
                    .on('filtered', function(chart, filters, e) {
                        ...
                        if ($scope.chart_show == 'map' && $scope.coming_from_tab) {
                            ...
                            if (chart.filters().length > 0) { resetbutton.style.visibility = 'visible'; } else { resetbutton.style.visibility = 'hidden'; }
                        }
                        ...
                        if ($scope.chart_show == 'map' && !$scope.coming_from_tab) {
                            ...
                            $scope.updateHTML(keyvalue);
                            var resetbutton = document.getElementsByClassName('reset-button')[0];
                            if ($scope.filters.length > 0) { resetbutton.style.visibility = 'visible'; } else { resetbutton.style.visibility = 'hidden'; }
                        }
                ...
                rowChart
                    ...
                    .on('filtered', function(chart, filters) {
                        ...
                        if ($scope.chart_show == 'row' && !$scope.coming_from_map) {
                            ...
                            $scope.updateHTML(keyvalue);
                            var resetbutton = document.getElementsByClassName('reset-button')[0];
                            if ($scope.filters.length > 0 /*|| $scope.row_filters_old.length > 0*/ ) { resetbutton.style.visibility = 'visible'; } else { resetbutton.style.visibility = 'hidden'; }
                        }
                    })
                ...
                $scope.row_text = function(color_range) {
                    ...
                    for (var i = 0; i < $('.dc-chart g.row').length; i++) {
                        ...
                        if (selection == 'deselected' && title.substring(0, 7) == 'No Data') { text.style.fill = 'white'; } else if (color_level <= 2 ||
                            selection == 'deselected' ||
                            !fill ||
                            parseInt(title.substring(0, 3)) < 3
                        ) { text.style.fill = 'black'; } else { text.style.fill = 'white'; };
                    }
                }
                ...
                $scope.sort = function(type) {
                    if (type === 'value') {
                        rowChart.ordering(function(d) { return isNaN(d.value.sum) ? 0 : -d.value.sum; });
                    } else if (type == 'name') {
                ...
                $scope.info = function(id) {
                    ...
                    if (!meta_icon[id]) { $scope.metric_icon = 'modules/dashboards/img/undefined.png'; } else { $scope.metric_icon = 'modules/dashboards/img/' + meta_icon[id]; }
                    //$scope.info_modal();
                        ...
                        if (i === 0) {
                            for (key in value) {
                                if (value.hasOwnProperty(key)) {
                                    if (meta_label[key]) { innerValue = meta_label[key]; } else if (meta_label[key.replace('_score', '')]) { innerValue = meta_label[key.replace('_score', '')] + ' (0-10)'; } else { innerValue = key; };
                ...
                $scope.mapShow = function() {
                    ...
                    if ($scope.filters.length == 0) {
                        ...
                        for (var i = 0; i < $scope.geom.features.length; i++) {
                            if ($scope.filters.indexOf($scope.geom.features[i].properties.pcode) > -1) {
